...
export class RaiPipe implements PipeTransform {
  ...
  transform(value: any, args?: any): any {
    ...
    switch (denomination.toLowerCase()) {
      default:
      case 'aibtc': return `${(value / this.mrai).toFixed(6)}${!hideText ? ' AIBTC' : ''}`;
      case 'mnano':
        ...
        if (hasRawValue) {
          ...
          return `${this.toFixed(newVal, this.precision)}${!hideText ? ' AIBTC' : ''}`;
        } else {
          return `${(value / this.mrai).toFixed(6)}${!hideText ? ' AIBTC' : ''}`;
        }
