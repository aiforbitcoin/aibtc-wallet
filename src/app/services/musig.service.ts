...
export class MusigService {
  ...
  aggregate(storedAccounts, runWithPubkeys = null) {
    ...
    for (let address of addresses) {
      ...
      if (!address.startsWith('aibtc_') && !address.startsWith('aibtc_') && !address.startsWith('nano_')) {
        throw new Error('AI-Bitcoin addresses must start with aibtc_');
      }
      address = address.split('_', 2)[1];
      try {
        const bytes = base32.decode(address);
        if (bytes.length !== 37) {
          throw new Error('Wrong AI-Bitcoin address length');
        }
        ...
        if (!this.util.array.equalArrays(bytes.subarray(32), checksum_)) {
          throw new Error('Invalid AI-Bitcoin address checksum');
        }
        pubkeys.push(pubkey);
      } catch (err_) {
          console.error(err_.toString());
          throw new Error('Invalid AI-Bitcoin address (bad character?)');
      }
    }
    ...
    const fullAddressFinal = 'aibtc_' + base32.encode(fullAddress);
