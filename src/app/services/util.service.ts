...
function getPublicAccountID(accountPublicKeyBytes, prefix = 'aibtc') {
...
function setPrefix(account, prefix = 'aibtc') {
  if (prefix === 'nano') {
    return account.replace('nano_', 'aibtc_');
  }  else if (prefix === 'xrb') {
    return account.replace('xrb_', 'aibtc_');
  }  else {
    return account.replace('nano_', 'aibtc_');
  }
}
