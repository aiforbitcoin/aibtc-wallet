...
export class AccountDetailsComponent implements OnInit, OnDestroy {
  ...
  remoteVisible = false;
  blockTypes: string[] = ['Send AI-Bitcoin', 'Change Representative'];
  ...
  async receiveReceivableBlock(receivableBlock) {
    ...
    if (createdReceiveBlockHash) {
      ...
      this.notifications.sendSuccess(`Successfully received AI-Bitcoin!`, { identifier: 'success-receive' });
  ...
  async generateSend() {
    ...
    if (!this.validateAmount()) return this.notifications.sendWarning(`Invalid AIBTC Amount`);
    ...
    if (from.balanceBN.minus(rawAmount).lessThan(0)) return this.notifications.sendError(`From account does not have enough AIBTC`);
    ...
    const blockData = {
      account: this.accountID.replace('nano_', 'aibtc_').toLowerCase(),
    ...
    const blockDataPrevious = {
      account: jsonBlock.account.replace('nano_', 'aibtc_').toLowerCase(),
