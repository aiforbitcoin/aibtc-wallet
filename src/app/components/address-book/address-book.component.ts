...
export class AddressBookComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  async saveNewAddress() {
    ...
    this.newAddressAccount = this.newAddressAccount.replace(/ /g, '').replace('nano_', 'aibtc_');
  ...
  exportAddressBookToFile() {
    const fileName = `AI-Bitcoin-AddressBook.json`;
    ...
    reader.onload = (event) => {
      ...
      try {
        ...
        if (!importData.length || (!importData[0].account && !importData[0].address)) {
          return this.notificationService.sendError(this.translocoService.translate('address-book.bad-import-data-make-sure-you-selected-a-aibtc-wallet-address-book'));
        }
