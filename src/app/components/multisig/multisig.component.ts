...
export class MultisigComponent implements OnInit {
  ...
  addAccount() {
    if (this.accountAddStatus !== 1) {
      ...
      this.notificationService.sendWarning('Invalid AI-Bitcoin address!', {identifier: 'account-invalid'});
      return;
    }
    if (this.storedAccounts.includes(this.accountAdd.replace('nano_', 'aibtc_').toLocaleLowerCase())) {
    ...
    this.storedAccounts.push(this.accountAdd.replace('nano_', 'aibtc_').toLocaleLowerCase());
    this.accountAdd = '';
  ...
  navigateAccount() {
    if (this.validateMultisig()) {
      ...
      this.notificationService.sendWarning('Invalid AI-Bitcoin account!');
    }
  }
